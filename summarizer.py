...
def chunk_big_file(audio_filename):
    audio = AudioSegment.from_mp3("temp/"+audio_filename+".mp3")
    ...
    for i in range(num_chunks):
        ...
        chunk.export(f"temp/chunk_{i}.mp3", format="mp3")
    ...
    if remaining_time > 0:
        ...
        chunk = audio[start_time * 1000:end_time * 1000]
        chunk.export(f"temp/chunk_{num_chunks}.mp3", format="mp3")
        num_chunks += 1
...
def cleanup(audio_filename, num_chunks):
    try:
        os.remove("temp/"+audio_filename+".mp3")
        os.remove("temp/"+audio_filename+"_mono.mp3")
    except:
        pass
    for i in range(num_chunks):
        try:
            os.remove(f"temp/chunk_{i}.mp3")
        except:
            continue
...
def main_process_youtube_channel():
    ...
    try:
        for video_number in range(CURRENT_VIDEO_PROCCESSING, len(youtube_videos)):
                print("processing video ", video_number)
                url = youtube_videos["Video url"][video_number]
                video_title = youtube_videos["Video Title"][video_number].replace(" ", "_").replace(":", "_").replace("|", "_").replace("?", "_").replace("!", "_").replace(";", "_").replace(",", "_").replace(".", "_").replace("/", "_")
                ...
                for chunk in range(num_chunks):
                    chunk_filename = f"temp/chunk_{chunk}.mp3"
                    try:
                        output = whisper_transcription(model, enc, chunk_filename)
                        with open("txt/"+video_title+"_transcription.txt", "a") as text_file:
                ...
                with open ("current_video_file.dat", "w") as current_video_file:
                    current_video_file.write(str(video_number+1))
...
def main():
    ...
    print("Transcribing audio... with", num_chunks, " chunks")
    for i in range(num_chunks):
        chunk_filename = f"temp/chunk_{i}.mp3"
        try:
            output = whisper_transcription(model, enc, chunk_filename)
            with open("temp/"+video_title+"_transcription.txt", "a") as text_file:
                text_file.write(output)
        except:
            continue

    print("Summarizing with GPT-4...")
    with open("temp/"+video_title+"_transcription.txt", "r") as file:
        data = file.read()
    summary = summarize_youtube(video_title, data)
    with open("temp/"+video_title+"_summary.txt", "w") as text_file:
